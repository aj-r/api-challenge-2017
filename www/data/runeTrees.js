runeTrees = [
    {
        "name": "Precision",
        "perkStyleId": 8000,
        "runeIds": [8005, 8008, 8021, 9101, 9111, 8009, 9104, 9105, 9103, 8014, 8017, 8299],
        "runes": [
            [
                {
                    "id": 8005,
                    "name": "Press the Attack",
                    "description": "Hitting an enemy champion 3 consecutive times makes them vulnerable, dealing bonus damage and causing them to take more damage from all sources for 6s.",
                    "keystone": true
                },
                {
                    "id": 8008,
                    "name": "Lethal Tempo",
                    "description": "1.5s after damaging a champion, gain a large amount of Attack Speed. Lethal Tempo allows you to temporarily exceed the attack speed limit.",
                    "keystone": true
                },
                {
                    "id": 8021,
                    "name": "Fleet Footwork",
                    "description": "Attacking and moving builds Energy stacks. At 100 stacks, your next attack heals you and grants increased MS.",
                    "keystone": true
                }
            ],
            [
                {
                    "id": 9101,
                    "name": "Overheal",
                    "description": "Excess healing on you becomes a shield."
                },
                {
                    "id": 9111,
                    "name": "Triumph",
                    "description": "Takedowns restore 12% of your missing health and grant an additional 25 gold."
                },
                {
                    "id": 8009,
                    "name": "Presence of Mind",
                    "description": "For 7s after gaining a level or takedown, any mana you spend is fully restored."
                }
            ],
            [
                {
                    "id": 9104,
                    "name": "Legend: Alacrity",
                    "description": "Takedowns on enemies grant permanent Attack Speed."
                },
                {
                    "id": 9105,
                    "name": "Legend: Tenacity",
                    "description": "Takedowns on enemies grant permanent Tenacity."
                },
                {
                    "id": 9103,
                    "name": "Legend: Bloodline",
                    "description": "Takedowns on enemies grant permanent Lifesteal."
                }
            ],
            [
                {
                    "id": 8014,
                    "name": "Coup de Grace",
                    "description": "Deal more damage to low health enemy champions."
                },
                {
                    "id": 8017,
                    "name": "Cut Down",
                    "description": "Deal more damage to champions with more max health than you."
                },
                {
                    "id": 8299,
                    "name": "Last Stand",
                    "description": "Deal more damage to champions while you are low on health."
                }
            ]
        ]
    },
    {
        "name": "Domination",
        "perkStyleId": 8100,
        "runeIds": [8112, 8124, 8128, 8126, 8139, 8143, 8136, 8120, 8138, 8135, 8134, 8105],
        "runes": [
            [
                {
                    "id": 8112,
                    "name": "Electrocute",
                    "description": "Hitting a champion with 3 separate attacks or abilities in 3 seconds deals bonus adaptive damage.",
                    "keystone": true
                },
                {
                    "id": 8124,
                    "name": "Predator",
                    "description": "Add an active effect to your boots that grants a large boost of MS and causes your next attack or ability to deal bonus adaptive damage.",
                    "keystone": true
                },
                {
                    "id": 8128,
                    "name": "Dark Harvest",
                    "description": "Champions, large minions, and large monsters drop soul essence on death. Touch souls to absorb them and deal bonus adaptive damage on your next attack based on total soul essence collected.",
                    "keystone": true
                }
            ],
            [
                {
                    "id": 8126,
                    "name": "Cheap Shot",
                    "description": "Deal bonus true damage to enemy champions with impaired movement or actions."
                },
                {
                    "id": 8139,
                    "name": "Taste of Blood",
                    "description": "Heal when you damage an enemy champion."
                },
                {
                    "id": 8143,
                    "name": "Sudden Impact",
                    "description": "Gain a burst of Lethality and Magic Penetration after using a dash, leap, blink, teleport, or when leaving stealth."
                }
            ],
            [
                {
                    "id": 8136,
                    "name": "Zombie Ward",
                    "description": "After killing a ward, a friendly Zombie Ward is raised in its place. When your wards expire, they also reanimate as Zombie Wards."
                },
                {
                    "id": 8120,
                    "name": "Ghost Poro",
                    "description": "When you enter brush, a poro appears. It will stay behind to give you vision."
                },
                {
                    "id": 8138,
                    "name": "Eyeball Collection",
                    "description": "Collect eyeballs for champion and ward takedowns. Gain permanent AD or AP, adaptive, for each eyeball, plus bonus upon collection completion."
                }
            ],
            [
                {
                    "id": 8135,
                    "name": "Ravenous Hunter",
                    "description": "Unique takedowns grant permanent healing from ability damage."
                },
                {
                    "id": 8134,
                    "name": "Ingenious Hunter",
                    "description": "Unique takedowns grant permanent Active Item CDR (includes Trinkets)."
                },
                {
                    "id": 8105,
                    "name": "Relentless Hunter",
                    "description": "Unique takedowns grant permanent out of combat MS."
                }
            ]
        ]
    },
    {
        "name": "Sorcery",
        "perkStyleId": 8200,
        "runeIds": [8214, 8229, 8230, 8224, 8226, 8243, 8210, 8234, 8233, 8237, 8232, 8236],
        "runes": [
            [
                {
                    "id": 8214,
                    "name": "Summon Aery",
                    "description": "Your attacks and abilities send Aery to a target, damaging enemies or shielding allies.",
                    "keystone": true
                },
                {
                    "id": 8229,
                    "name": "Arcane Comet",
                    "description": "Damaging a champion with an ability hurls a damaging comet at their location.",
                    "keystone": true
                },
                {
                    "id": 8230,
                    "name": "Phase Rush",
                    "description": "Hitting an enemy champion with 3 separate attacks or abilities grants a burst of MS.",
                    "keystone": true
                }
            ],
            [
                {
                    "id": 8224,
                    "name": "Nullifying Orb",
                    "description": "Gain a magic damage shield when taken to low health by magic damage."
                },
                {
                    "id": 8226,
                    "name": "Manaflow Band",
                    "description": "Periodically, your next ability used has its mana or energy cost refunded and restores some of your missing mana or energy."
                },
                {
                    "id": 8243,
                    "name": "The Ultimate Hat",
                    "description": "Your ultimate's cooldown is reduced. Each time you cast your ultimate, its cooldown is further reduced."
                }
            ],
            [
                {
                    "id": 8210,
                    "name": "Transcendence",
                    "description": "Gain 10% CDR when you reach level 10. Excess CDR becomes AP or AD, adaptive."
                },
                {
                    "id": 8234,
                    "name": "Celerity",
                    "description": "Gain 3% extra MS. Gain extra AP or AD, adaptive, based on your bonus MS."
                },
                {
                    "id": 8233,
                    "name": "Absolute Focus",
                    "description": "While above 70% health, gain extra adaptive damage."
                }
            ],
            [
                {
                    "id": 8237,
                    "name": "Scorch",
                    "description": "Your first ability hit every 20s burns champions."
                },
                {
                    "id": 8232,
                    "name": "Waterwalking",
                    "description": "Gain MS and AP or AD, adaptive, in the river."
                },
                {
                    "id": 8236,
                    "name": "Gathering Storm",
                    "description": "Gain increasing amounts of AD or AP, adaptive, over the course of the game."
                }
            ]
        ]
    },
    {
        "name": "Resolve",
        "perkStyleId": 8300,
        "runeIds": [8437, 8439, 8465, 8242, 8446, 8463, 8430, 8435, 8429, 8451, 8453, 8444],
        "runes": [
            [
                {
                    "id": 8437,
                    "name": "Grasp of the Undying",
                    "description": "Every 4s, your next attack on a champion deals bonus magic damage, heals you, and permanently increases your health.",
                    "keystone": true
                },
                {
                    "id": 8439,
                    "name": "Aftershock",
                    "description": "After immobilizing an enemy champion, gain defenses and later deal a burst of adaptive damage around you.",
                    "keystone": true
                },
                {
                    "id": 8465,
                    "name": "Guardian",
                    "description": "Guard allies you cast spells on and those that are very nearby. If you or a guarded ally would take damage, you're both hasted and granted a shield.",
                    "keystone": true
                }
            ],
            [
                {
                    "id": 8242,
                    "name": "Unflinching",
                    "description": "After casting a Summoner Spell, gain Tenacity and Slow Resistance for a short duration. Additionally, gain Tenacity and Slow Resistance for each Summoner Spell on cooldown."
                },
                {
                    "id": 8446,
                    "name": "Demolish",
                    "description": "Charge up a powerful attack against a tower while near it."
                },
                {
                    "id": 8463,
                    "name": "Font of Life",
                    "description": "Impairing the movement of an enemy champion marks them. Your allies heal when attacking champions you've marked."
                }
            ],
            [
                {
                    "id": 8430,
                    "name": "Iron Skin",
                    "description": "Gain +5 Armor. Heals, including consumables, increase your Armor by 5% temporarily."
                },
                {
                    "id": 8435,
                    "name": "Mirror Shell",
                    "description": "Gain +6 Magic Resist. Heals, including consumables, increase your Magic Resist by 5% temporarily."
                },
                {
                    "id": 8429,
                    "name": "Conditioning",
                    "description": "After 10 min, gain +8 Armor and +8 Magic Resist and increase your Armor and Magic Resist by 5%."
                }
            ],
            [
                {
                    "id": 8451,
                    "name": "Overgrowth",
                    "description": "Gain additional permanent max health when minions or monsters die near you."
                },
                {
                    "id": 8453,
                    "name": "Revitalize",
                    "description": "Heals and shields you cast or receive are 5% stronger and increased by an additional 10% on low health targets."
                },
                {
                    "id": 8444,
                    "name": "Second Wind",
                    "description": "After taking damage from an enemy champion, heal back some missing health over time."
                }
            ]
        ]
    },
    {
        "name": "Inspiration",
        "perkStyleId": 8400,
        "runeIds": [8326, 8351, 8359, 8306, 8345, 8313, 8304, 8321, 8316, 8347, 8410, 8339],
        "runes": [
            [
                {
                    "id": 8326,
                    "name": "Unsealed Spellbook",
                    "description": "Exchange Summoner Shards at the shop to change your Summoner Spells during game. Your Summoner Spells have reduced cooldowns.",
                    "keystone": true
                },
                {
                    "id": 8351,
                    "name": "Glacial Augment",
                    "description": "Your first attack against an enemy champion slows them (per unit cooldown). Slowing champions with active items shoots a freeze ray at them, creating a lingering slow zone.",
                    "keystone": true
                },
                {
                    "id": 8359,
                    "name": "Kleptomancy",
                    "description": "After using an ability, your next attack will grant bonus gold if used on a champion. There's a chance you'll also gain a consumable.",
                    "keystone": true
                }
            ],
            [
                {
                    "id": 8306,
                    "name": "Hextech Flashtraption",
                    "description": "While Flash is on cooldown it is replaced by Hexflash. Hexflash: Channel, then blink to a new location."
                },
                {
                    "id": 8345,
                    "name": "Biscuit Delivery",
                    "description": "Gain a free Biscuit every 3 min, until 12 min. Consuming a Biscuit permanently increases your max mana and restores health and mana."
                },
                {
                    "id": 8313,
                    "name": "Perfect Timing",
                    "description": "Gain a free Stopwatch. Stopwatch has a one-time use Stasis effect."
                }
            ],
            [
                {
                    "id": 8304,
                    "name": "Magical Footwear",
                    "description": "You get free boots at 10 min but you cannot buy boots before then. Each takedown you get makes your boots come 30s sooner."
                },
                {
                    "id": 8321,
                    "name": "Future's Market",
                    "description": "You can enter debt to buy items."
                },
                {
                    "id": 8316,
                    "name": "Minion Dematerializer",
                    "description": "Start the game with 6 Minion Dematerializers. Killing minions with the item gives permanent bonus damage vs. that minion type."
                }
            ],
            [
                {
                    "id": 8347,
                    "name": "Cosmic Insight",
                    "description": "+5% CDR, +5% Max CDR, +5% Summoner Spell CDR, +5% Item CDR"
                },
                {
                    "id": 8410,
                    "name": "Approach Velocity",
                    "description": "Bonus MS towards nearby ally champions that are movement impaired or enemy champions that you impair."
                },
                {
                    "id": 8339,
                    "name": "Celestial Body",
                    "description": "+100 Health permanently, -10% damage to champions and monsters until 10 min"
                }
            ]
        ]
    }
]
